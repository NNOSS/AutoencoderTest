...
def conv_dec_net(encoded, nx, ny, reuse = None, n_out=1):
    with tf.variable_scope("u_net", reuse = None):
...
if(convolutional):
  ...
  with tf.variable_scope("u_net", reuse=None):
        ...
        flat3 = FlattenLayer(conv3, name = 'flatten')
        enc3 = DenseLayer(flat3, encoding_size, act = tf.nn.relu, name = 'hidden_encode')
        dec3 = DenseLayer(enc3, 784, act = tf.nn.relu, name = 'hidden_decode')
        shape3 = ReshapeLayer(dec3, (-1, 7, 7, 16), name = 'unflatten')
        ...
        #up1 = ConcatLayer([up1, conv1] , 3, name='concat1')
        conv1 = Conv2d(up1, 4, (3, 3), act=tf.nn.relu, name='uconv1_1')
        conv1 = Conv2d(conv1, 1, (1, 1), act=tf.nn.sigmoid, name='uconv1')

  #encoding = conv_enc_net(X, reuse = False)
  enc_outs = enc3.outputs
  variable_summaries(enc_outs)
  f = conv1.outputs#conv_dec_net(encoding, nx, ny, reuse = None).outputs
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        if (step % encoding_print_step == 0):
          dec_summy = sess.run(dec_summary, feed_dict={enc3.outputs: 30*np.eye(encoding_size)}) # Y is same as X
